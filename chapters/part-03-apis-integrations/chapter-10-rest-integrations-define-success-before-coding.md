...
- Calculate and interpret request count, valid business-result success rate, error rate, and normalized error counts.
...
The scenario measures **ClearVerify REST identity verification**. HTTP 200 is transport evidence, not proof of business success. Conversely, a transient transport failure need not end the workflow.
...
This formerly planned scope is now implemented as ClearVerify REST identity verification. All vendors, operations, observations, members, and metrics are fictional and synthetic.

## Metrics to measure

- request count, valid business-result success rate, error rate, and normalized error counts.
...
The planned exercise is fulfilled by `scripts/measure_api_reliability.py` and its reusable implementation. No real REST, SOAP, payment, identity, or core service is contacted.
...
HTTP 200 is transport evidence, not proof of business success. Conversely, a transient transport failure need not end the workflow.
